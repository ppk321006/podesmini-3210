...
const Index = () => {
  ...
  const chartData = convertProgressDataToChartData(progressData);
    ...
    <div className="container mx-auto py-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {totals ? Math.round((totals.total_padi + totals.total_palawija) / (totals.padi_target + totals.palawija_target) * 100) : 0}%
            </div>
            ...
            <div className="h-2 bg-gray-200 rounded-full mt-2">
              <div 
                ...
                style={{ 
                  width: totals ? 
                    `${Math.min(100, Math.round((totals.total_padi + totals.total_palawija) / (totals.padi_target + totals.palawija_target) * 100))}%` 
                    : '0%' 
                }}
              />
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {totals && totals.padi_target > 0 ? Math.round((totals.total_padi / totals.padi_target) * 100) : 0}%
            </div>
            <div className="text-xs text-muted-foreground">
            ...
            <div className="h-2 bg-gray-200 rounded-full mt-2">
              <div 
                className="h-2 bg-green-500 rounded-full" 
                style={{ 
                  width: totals && totals.padi_target > 0 ? 
                    `${Math.min(100, Math.round((totals.total_padi / totals.padi_target) * 100))}%` 
                    : '0%' 
                }}
              />
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {totals && totals.palawija_target > 0 ? Math.round((totals.total_palawija / totals.palawija_target) * 100) : 0}%
            </div>
            <div className="text-xs text-muted-foreground">
            ...
            <div className="h-2 bg-gray-200 rounded-full mt-2">
              <div 
                className="h-2 bg-green-500 rounded-full" 
                style={{ 
                  width: totals && totals.palawija_target > 0 ? 
                    `${Math.min(100, Math.round((totals.total_palawija / totals.palawija_target) * 100))}%` 
                    : '0%' 
                }}
              />
            </div>
